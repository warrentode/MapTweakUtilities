...
local assets = {
    Asset("ANIM", "anim/portable_firepit.zip"), -- you can make a cold build/anim if you want
}

local prefabs = {
    "coldfirefire",
    "charcoal",
    "portablecoldfirepit_item",
}
...
local function ChangeToItem(inst)
    local item = SpawnPrefab("portablecoldfirepit_item")
...
local function updatefuelrate(inst)
    inst.components.fueled.rate = TheWorld.state.israining and inst.components.rainimmunity == nil and 1 or 1
end
...
local function fn()
    ...
    inst:SetDeploySmartRadius(DEPLOYSPACING_RADIUS[DEPLOYSPACING.ONEPOINTFIVE] / 2)
    ...
    inst.components.inspectable:SetNameOverride("coldfirepit")
...
local function itemfn()
    ...
    inst.components.inventoryitem.imagename = "portablefirepit_item"
...
return Prefab("portablecoldfirepit", fn, assets, prefabs),
MakePlacer("portablecoldfirepit_item_placer", "portable_firepit", "portable_firepit", "placer"),
Prefab("portablecoldfirepit_item", itemfn, assets, prefabs_item)