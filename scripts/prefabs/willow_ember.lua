...
local function OnDropped(inst)
    if inst.components.stackable ~= nil and inst.components.stackable:IsStack() then
        ...
        if num > 0 then
            for _ = 1, math.min(num, inst.components.stackable:StackSize()) do
...
local function TryBurstFire(inst, doer, _)
...
local function TryFireFrenzy(inst, doer, _)
...
local function ReticuleFireThrowTargetFn(_)
...
local function burst_reticule_update_position_function(inst, _, reticule, _, _, _)
    local inventoryitem = inst.replica.inventoryitem
    local owner = inventoryitem and inventoryitem:IsGrandOwner(ThePlayer) and ThePlayer
...
local function single_reticule_update_position_function(inst, _, reticule, _, _, _)
...
local function line_reticule_update_position_function(inst, _, reticule, ease, smoothing, dt)
    local inventoryitem = inst.replica.inventoryitem
...
--- making this global to access it from modmain
--- this is to minimize potential problems with other mods while also allowing my feature to be toggable
updatespells = function(inst, owner)
...
local function fn()
    ...
    if not TheWorld.ismastersim then
        ...
        inst._onskillrefresh_client = function(_)
            DoClientUpdateSpells(inst, true)
        end
...
local function Buff_OnExtended(inst, _)
